<?php 
	...
	$limit = 2;
	$offset = ($hal - 1)*$limit;
	$hasil = mysql_query("select * from content where kode_kategori = '".$kode_kategori."' order by kode_content desc limit $offset,$limit;");	
	$get_total = mysql_query("select count(*) as total from content where kode_kategori = '".$kode_kategori."'");
	$total_artikel = 0;
    ...
    <div id="templatemo_content">
    	...
    	<div class="post_section">
		   ...
		   <?php echo $data["judul_content"]; ?></a></h2>            
            <?php echo $data["tanggal"]; ?> by <a href="_blank" target="_parent"><?php echo $data["penulis"]; ?></a>            
        ...
        <div>
        	<?php if($hal > 1){ ?>
        	<a href="<?php echo $_SERVER['PHP_SELF']; ?>?page=kategori&kode=<?php echo $kode_kategori; ?>&hal=<?php echo ($hal-1); ?>" style="font-size:18px; text-decoration:none; float:left;">&lt;&lt;&nbsp;&nbsp;Prev</a>
           	<?php 
				}if($hal < $total_page){
			?>
            
            <a href="<?php echo $_SERVER['PHP_SELF']; ?>?page=kategori&kode=<?php echo $kode_kategori; ?>&hal=<?php echo ($hal+1); ?>" style="font-size:18px; text-decoration:none; float:right;">Next&nbsp;&nbsp;&gt;&gt;</a>
            <?php } ?>
        </div>
